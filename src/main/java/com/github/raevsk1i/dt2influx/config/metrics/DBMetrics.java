...
public class DBMetrics {
    ...
    public static final List<MetricDefinition> generic_metrics = List.of(
            new MetricDefinition(
                    MetricMeasurement.MEMORY,
                    "working_set_size",
                    "builtin:tech.generic.mem.workingSetSize:filter(or(in(\"dt.entity.process_group_instance\",entitySelector(\"type(PROCESS_GROUP_INSTANCE) AND fromRelationship.isProcessOf(type(HOST),entityName.contains(~\"REPLACE_IT_FOR_DB_HOST~\"))\")))):splitBy(\"dt.entity.process_group_instance\"):names"),
            new MetricDefinition(
                    MetricMeasurement.CPU,
                    "cpu_usage",
                    "builtin:tech.generic.cpu.usage:filter(or(in(\"dt.entity.process_group_instance\",entitySelector(\"type(PROCESS_GROUP_INSTANCE) AND fromRelationship.isProcessOf(type(HOST),entityName.contains(~\"REPLACE_IT_FOR_DB_HOST~\"))\")))):splitBy(\"dt.entity.process_group_instance\"):names")
